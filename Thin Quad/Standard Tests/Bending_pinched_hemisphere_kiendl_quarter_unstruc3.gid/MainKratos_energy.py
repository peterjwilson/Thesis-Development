...
energyoutput = open("energy.txt", "w")
...
while(time <= end_time):
    ...
    for element in main_model_part.Elements:
        ...
        tempbend = []
        tempshear = []
        tempmem = element.GetValuesOnIntegrationPoints(SHELL_ELEMENT_MEMBRANE_ENERGY, proc_info)
        tempbend = element.GetValuesOnIntegrationPoints(SHELL_ELEMENT_BENDING_ENERGY, proc_info)
        tempshear = element.GetValuesOnIntegrationPoints(SHELL_ELEMENT_SHEAR_ENERGY, proc_info)
        for gauss_point in range(4):
            membrane_energy += tempmem[gauss_point][0]
            bending_energy += tempbend[gauss_point][0]
            shear_energy += tempshear[gauss_point][0]

    total_energy = membrane_energy + bending_energy + shear_energy
    energyoutput.write(str(membrane_energy/total_energy)+"\n")
    energyoutput.write(str(bending_energy/total_energy)+"\n")
    energyoutput.write(str(shear_energy/total_energy)+"\n")
    energyoutput.write(str(total_energy)+"\n")
