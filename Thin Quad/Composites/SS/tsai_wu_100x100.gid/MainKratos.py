...
tsaifile = open("tsai_wu_rf.txt", "w")
...
while(time <= end_time):
    ...
    for element in main_model_part.Elements:
        ...
        for node in element.GetNodes():
            x += node.X/4.0
            y += (node.Y/4.0)
        if y > 0.0 and y < 10.0:    #only take elements along the bottom boundary if y > 95.0 and y < 100:
            # Curvatures
            # Bottom surface VM stress
            tsaifile.write(str(x) + "\t" + str(y))
            strain_result = []
            vm = 0.0
            strain_result = element.GetValuesOnIntegrationPoints(TSAI_WU_RESERVE_FACTOR, proc_info)
            for gauss_point in range(4):    #average gauss point values into central one
                vm += strain_result[gauss_point][0]/4.0
            tsaifile.write("\t" + str(vm))
            tsaifile.write(("\n"))
...
tsaifile.close() 
stress_bottom_surface_file.close()   
