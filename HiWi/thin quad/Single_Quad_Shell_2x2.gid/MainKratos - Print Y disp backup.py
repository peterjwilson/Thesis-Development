...
def setLoad(model_part):
    for node in model_part.Nodes:
        node.SetSolutionStepValue(POINT_LOAD_Y,0,1)

def printDisp(model_part):
    for node in model_part.Nodes:
        print('\nY Displacement is:\n',node.GetSolutionStepValue(DISPLACEMENT_Y,0),"\n")
...
setLoad(main_model_part.GetSubModelPart("PointLoad3D_Neumann"))
...
while(time <= end_time):
    ...
    printDisp(main_model_part.GetSubModelPart("PointLoad3D_Neumann"))
    for process in list_of_processes:
