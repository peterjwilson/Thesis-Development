...
def ApplyLoad(initial_value, model_part, time):
    for node in model_part.Nodes:
        ...
        node.SetSolutionStepValue(DISPLACEMENT_Z,0,value)
...
while(time <= end_time):
    ...
    ApplyLoad(0.005, main_model_part.GetSubModelPart("DISPLACEMENT_axial_load"), time)
        
    solver.Solve()
    
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_axial_load").Nodes:
        fol.write(str(node.GetSolutionStepValue(REACTION_Z,0)) +  "\n")
        print("Axial Load = \t",str(node.GetSolutionStepValue(REACTION_Z,0)))
