...
while(time <= end_time):
    ...
    ApplyLoad(-2.5, main_model_part.GetSubModelPart("DISPLACEMENT_sym"), time)
        
    solver.Solve()
    
    av_load = 0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_sym").Nodes:
        av_load +=node.GetSolutionStepValue(REACTION_Z,0)
    fol.write(str(av_load/2.0) +  "\n")
