...
while(time <= end_time):
    ...
    ApplyLoad(4000000, main_model_part.GetSubModelPart("PointLoad3D_load"), time)
        
    solver.Solve()
    
    for node in main_model_part.GetSubModelPart("PointLoad3D_load").Nodes:
        fol.write(str(node.GetSolutionStepValue(POINT_LOAD_Z,0)) +  "\n")
        print("Load = ",str(node.GetSolutionStepValue(POINT_LOAD_Z,0)))
    
    for node in main_model_part.GetSubModelPart("PointLoad3D_load").Nodes:
        fodz.write(str(node.GetSolutionStepValue(DISPLACEMENT_Z,0)) +  "\n")
        print("Disp = ",str(node.GetSolutionStepValue(DISPLACEMENT_Z,0)))
        fodx.write(str(node.GetSolutionStepValue(DISPLACEMENT_X,0)) +  "\n")
        print("Disp = ",str(node.GetSolutionStepValue(DISPLACEMENT_X,0)))
