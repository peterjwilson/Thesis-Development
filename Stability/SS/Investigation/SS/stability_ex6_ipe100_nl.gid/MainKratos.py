...
def ApplyLoad(initial_value, model_part, time):
    for node in model_part.Nodes:
        ...
        node.SetSolutionStepValue(POINT_LOAD_Z,0,value)
...
fodz = open("displacementz.txt", "w")
fodx = open("displacementx.txt", "w")
fol = open("load.txt", "w")
...
while(time <= end_time):
    ...
    ApplyLoad(500000, main_model_part.GetSubModelPart("PointLoad3D_axial_load"), time)
        
    solver.Solve()
    
    for node in main_model_part.GetSubModelPart("PointLoad3D_axial_load").Nodes:
        fol.write(str(node.GetSolutionStepValue(POINT_LOAD_Z,0)) +  "\n")
        print("Axial Load = \t",str(node.GetSolutionStepValue(POINT_LOAD_Z,0)))
        fodz.write(str(node.GetSolutionStepValue(DISPLACEMENT_Z,0)) +  "\n")
        print("Axial Disp = \t",str(node.GetSolutionStepValue(DISPLACEMENT_Z,0)))
 
    for node in main_model_part.GetSubModelPart("PointLoad3D_lateral_load").Nodes:
        print("Lat Load = \t",str(node.GetSolutionStepValue(POINT_LOAD_X,0)))
        fodx.write(str(node.GetSolutionStepValue(DISPLACEMENT_X,0)) +  "\n")
        print("Lat Disp = \t",str(node.GetSolutionStepValue(DISPLACEMENT_X,0)))
