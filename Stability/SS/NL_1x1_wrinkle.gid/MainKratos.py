...
def ApplyForce( model_part, time):
    for node in model_part.Nodes:       
        force = 0
        if time > 0.499 and time < 0.501:
            force = 0
            print("Applying force!")
        node.SetSolutionStepValue(POINT_LOAD_Z,0,force)
...
while(time <= end_time):
    ...
    print("Time = ", time)    
    ApplyDisp(main_model_part.GetSubModelPart("DISPLACEMENT_top_edge"), time)
    
    ApplyForce(main_model_part.GetSubModelPart("PointLoad3D_point_load"), time)
