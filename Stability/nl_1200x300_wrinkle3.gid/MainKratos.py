...
def ApplyDisp( model_part, time):
    for node in model_part.Nodes:      
        value = 0.015*time
        node.SetSolutionStepValue(DISPLACEMENT_X,0,value)
...
dispx = open("displacementx.txt", "w")
dispz = open("displacementz.txt", "w")
loadx = open("loadx.txt", "w")
loady = open("loady.txt", "w")
disppointz = open("displacement_point_z.txt", "w")
...
while(time <= end_time):
    ...
    print("Time = ", time)    
    ApplyDisp(main_model_part.GetSubModelPart("DISPLACEMENT_top"), time)
        
    solver.Solve()
    load_x = 0
    load_y = 0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        load_x += node.GetSolutionStepValue(REACTION_X,0)
        load_y += node.GetSolutionStepValue(REACTION_Y,0)
    loadx.write(str(load_x) +  "\n")
    loady.write(str(load_y) +  "\n")
    
    disp_x = 0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        disp_x = node.GetSolutionStepValue(DISPLACEMENT_X,0)
        dispx.write(str(disp_x) + "\n")
        break
    
    max_z = 0
    for node in main_model_part.Nodes:
        temp = abs(node.GetSolutionStepValue(DISPLACEMENT_Z,0))
        if temp > max_z:
            max_z = temp
    print("|Max Z| = ",max_z,"\n")
    dispz.write(str(max_z) + "\n")
    
    for node in main_model_part.GetSubModelPart("PointLoad3D_load").Nodes:
        disp_zp = node.GetSolutionStepValue(DISPLACEMENT_Z,0)
        disppointz.write(str(disp_zp) + "\n")
        break
