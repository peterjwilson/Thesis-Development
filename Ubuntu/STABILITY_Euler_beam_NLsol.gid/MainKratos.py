...
while(time <= end_time):
    ...
    ApplyLoad(-0.1, main_model_part.GetSubModelPart("DISPLACEMENT_top"), time)
        
    solver.Solve()
    
    reaction_tot = 0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        reaction_tot +=node.GetSolutionStepValue(REACTION_Z,0)
    fol.write(str(reaction_tot) +  "\n")
    
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        fod.write(str(node.GetSolutionStepValue(DISPLACEMENT_Z,0)) + "\n")
        break
