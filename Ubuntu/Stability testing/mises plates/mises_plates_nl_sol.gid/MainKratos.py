...
while(time <= end_time):
    ...
    ApplyLoad(-1.0, main_model_part.GetSubModelPart("DISPLACEMENT_top"), time)
        
    solver.Solve()
    load = 0
    counter = 0.0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        load += node.GetSolutionStepValue(REACTION_Z,0)
        counter += 1.0
    fol.write(str(load/counter) +  "\n")
    
    disp = 0.0
    counter = 0.0
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_top").Nodes:
        disp += node.GetSolutionStepValue(DISPLACEMENT_Z,0)
        counter += 1.0
    fod.write(str(disp/counter) + "\n")
