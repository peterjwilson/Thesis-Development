...
#import define_output
parameter_file_struc = open("ProjectParameters_struc.json",'r')
ProjectParameters_struc = Parameters( parameter_file_struc.read())
...
solver_module_struc = __import__(ProjectParameters_struc["solver_settings"]["solver_type"].GetString())
solver_struc = solver_module_struc.CreateSolver(main_model_part,ProjectParameters_struc["solver_settings"])


#add variables (always before importing the model part) (it must be integrated in the ImportModelPart)
# if we integrate it in the model part we cannot use combined solvers
solver.AddVariables()
solver_struc.AddVariables()

#read model_part (note: the buffer_size is set here) (restart can be read here)
#solver.ImportModelPart()
#solver_struc.ImportModelPart()
#solver_struc.computing_model_part_name = "Parts_surface"
solver_struc.ImportModelPart()
solver.computing_model_part_name = "Parts_surface"

#add dofs (always after importing the model part) (it must be integrated in the ImportModelPart)
# if we integrate it in the model part we cannot use combined solvers
solver.AddDofs()
solver_struc.AddDofs()
...
while(time <= end_time):
    ...
    print("==== TOP DISPLACEMENTS ====")
    for node in main_model_part.GetSubModelPart("PointLoad3D_top").Nodes:
        print("Disp Z = ", node.GetSolutionStepValue(DISPLACEMENT_Z,0))
...
#print("Estimated LPB load =:\n",math.sqrt(main_model_part.ProcessInfo[EIGENVALUE_VECTOR][0])*1e6)
