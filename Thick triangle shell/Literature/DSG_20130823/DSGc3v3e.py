...
#kirchhoffDispField_xi = a1 + a2*xi*a +a3*eta*b + a4*xi**2*a + 0.5*(a5+a6)*xi*eta*a*b + a7*eta**2*b
#kirchhoffDispField_eta = a1 + a2*xi*d +a3*eta*c + a4*xi**2*d + 0.5*(a5+a6)*xi*eta*d*c + a7*eta**2*c
...
#rmDispField_xi = a8*xi*a + a9*eta*b #no bubble
#rmDispField_eta = a8*xi*d + a9*eta*c #no bubble

moderatorDispField = 0.5*(a5-a6)*xi*eta
moderatorDispField_xi = 0.5*(a5-a6)*xi*eta
moderatorDispField_eta = 0.5*(a5-a6)*xi*eta
C = a5 - a6
...
#PX = kirchhoffDispField_xi + moderatorDispField_xi
#PY = kirchhoffDispField_eta - moderatorDispField_eta
...
#    GX = rmDispField_xi + moderatorDispField_xi
#    GY = rmDispField_eta - moderatorDispField_eta
...
eq2 = W2 - (shearGapField.subs(xi,x2)).subs(eta,y2) #x2, y2
eq3 = W3 - (shearGapField.subs(xi,x3)).subs(eta,y3) #x3, y3
...
#Construct the X shear rotation field
GX_rm = rmDispField_u
GX_rm = eta*(b*(0.5*PY1 + 0.5*PY3) - 1.0*W1 + 1.0*W3) + xi*(a*(0.5*PX1 + 0.5*PX2) - 1.0*W1 + 1.0*W2)
GX_mod = moderatorDispField_u
GX_mod = eta*xi*(a*(-0.5*PX1 + 0.5*PX3) + b*(0.5*PY1 - 0.5*PY2))
GX_u = GX_rm + GX_mod

print("\n rmDispField",rmDispField_u)
print("\n moderatorDispField",moderatorDispField_u)
...
GY_rm = rmDispField_u
GY_rm = eta*(c*(0.5*PY1 + 0.5*PY3) - 1.0*W1 + 1.0*W3) + xi*(d*(0.5*PX1 + 0.5*PX2) - 1.0*W1 + 1.0*W2)
GY_mod = moderatorDispField_u
GY_mod = eta*xi*(d*(-0.5*PX1 + 0.5*PX3) + c*(0.5*PY1 - 0.5*PY2))
GY_u = GY_rm - GY_mod
...
#rmShearField_X_u = sp.diff(GX_u,xi)*dxi_dx + sp.diff(GX_u,eta)*deta_dx + sp.diff(GY_u,xi)*dxi_dx + sp.diff(GY_u,eta)*deta_dx
#rmShearField_Y_u = sp.diff(GY_u,xi)*dxi_dy + sp.diff(GY_u,eta)*deta_dy + sp.diff(GX_u,xi)*dxi_dy + sp.diff(GX_u,eta)*deta_dy
