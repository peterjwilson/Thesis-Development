...
# Section 1 ---------------------------------------------------
PHI = a1 + a2*alpha +a3*beta + a4*alpha**2 + 0.5*(a5+a6)*alpha*beta + a7*beta**2

#GAM = a8*x + a9*y - Bub*x*y #generic bubble mode
GAM = a8*alpha + a9*beta - (a8+a9)*alpha*beta #generic bubble mode
#GAM = a8*x + a9*y #no bubble

PD = 0.5*(a5-a6)*alpha*beta
C = a5 - a6
...
if(is_normal_DSG):
    ...
    GX = a8*alpha + a9*beta #normal DSG
    GY = a8*alpha + a9*beta #normal DSG
else:
    GX = GAM + PD
    GY = GAM - PD
...
eq1 = W1 - (W.subs(alpha,x1)).subs(beta,y1)
eq2 = W2 - (W.subs(alpha,x2)).subs(beta,y2)
eq3 = W3 - (W.subs(alpha,x3)).subs(beta,y3)

eq4 = PX1 - (px.subs(alpha,x1)).subs(beta,y1)
eq5 = PX2 - (px.subs(alpha,x2)).subs(beta,y2)
eq6 = PX3 - (px.subs(alpha,x3)).subs(beta,y3)

eq7 = PY1 - (py.subs(alpha,x1)).subs(beta,y1)
eq8 = PY2 - (py.subs(alpha,x2)).subs(beta,y2)
eq9 = PY3 - (py.subs(alpha,x3)).subs(beta,y3)
...
GX_u = GX_u.subs([(alpha,((x2*y3-x3*y2) + x*(y2-y3) + y*(x3-x2))/detJ),(beta,((x3*y1-x1*y3) + x*(y3-y1) + y*(x1-x3))/detJ)])

GY_u = GY_u.subs([(alpha,((x2*y3-x3*y2) + x*(y2-y3) + y*(x3-x2))/detJ),(beta,((x3*y1-x1*y3) + x*(y3-y1) + y*(x1-x3))/detJ)])
...
print("\n\n\nSymbolic B Matrix ( cartesian space, detJ taken out) = ")
