...
#        ypos = float(node.Y)
#        print("Xpos = ",xpos)
#        value = initial_value*sin(mypi*xpos/a)*sin(mypi*ypos/b)
#        value = initial_value
#        node.SetSolutionStepValue(SURFACE_LOAD,0,value)
#    for element in model_part.Elements:
#        center_x = 0.0
#        center_y = 0.0
#        for i in range(3):
#            node = element.GetNode(i)
#            center_x += 1.0/3.0*node.X
#            center_y += 1.0/3.0*node.Y
#        a = 200.0
#        b = 200.0
#        value = initial_value*np.sin(np.pi*center_x/a)*np.sin(np.pi*center_y/b)
...
while(time <= end_time):
    ...
    ApplyLoad(main_model_part.GetSubModelPart("Parts_surface"))
    for process in list_of_processes:
