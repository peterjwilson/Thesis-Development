...
thru_result = open("thru_result.txt", "w")
...
while(time <= end_time):
    ...
    for element in main_model_part.Elements:
        ...
        for node in element.GetNodes():
            x += node.X/3.0
            y += (node.Y/3.0)
        if x > 6.25 and x < 6.5:    #only take elements along the top boundary if y > 95.0 and y < 100:
            print("Y = ",y)
            counter = counter + 1
            # Curvatures
            #curvaturefile.write(str(x) + "\t" + str(y))
            tsai = element.GetValuesOnIntegrationPoints(TSAI_WU_RESERVE_FACTOR, proc_info)
            print("tsai = ",tsai[0])
            strain_result = []
    ...
    for i in range (9):
                    result_av[i] /= counter
    for i in range(9):
        thru_result.write(str(strain_av[i])+ "\t")
        thru_result.write(("\n"))
