...
stress_top_surface_bot_edge_file = open("stress_top_surface_bot_edge.txt", "w")
...
while(time <= end_time):
    ...
    for element in main_model_part.Elements:
        ...
        if y > 98.0 and y < 100:    #only take elements along the top boundary
            ...
            #print("SHELL_CURVATURE_GLOBAL")
            for gauss_point in range(1):    #average gauss point values into central one
                for i in range (9):
                    strain_av[i] += strain_result[gauss_point][i]
            for i in range(9):
            ...
            strain_av = [0,0,0,0,0,0,0,0,0]
            #print("SHELL_ORTHOTROPIC_STRESS_TOP_SURFACE_GLOBAL")
            for gauss_point in range(1):    #average gauss point values into central one
                for i in range (9):
                    strain_av[i] += strain_result[gauss_point][i]/1.0
            for i in range(9):
            ...
            strain_av = [0,0,0,0,0,0,0,0,0]
            #print("SHELL_ORTHOTROPIC_STRESS_BOTTOM_SURFACE_GLOBAL")
            for gauss_point in range(1):    #average gauss point values into central one
                for i in range (9):
                    strain_av[i] += strain_result[gauss_point][i]/1.0
            for i in range(9):
...
stress_top_surface_bot_edge_file.close()
