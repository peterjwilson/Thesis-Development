...
while(time <= end_time):
    ...
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_node1").Nodes:
        tempVec.append(node.GetSolutionStepValue(DISPLACEMENT_Z,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_X,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_Y,0))
        
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_node2").Nodes:
        tempVec.append(node.GetSolutionStepValue(DISPLACEMENT_Z,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_X,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_Y,0))
        
    for node in main_model_part.GetSubModelPart("DISPLACEMENT_node3").Nodes:
        tempVec.append(node.GetSolutionStepValue(DISPLACEMENT_Z,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_X,0))
        tempVec.append(node.GetSolutionStepValue(ROTATION_Y,0))
        
    for i in range(9):
        disp.write(str(tempVec[i]) + "\n")
